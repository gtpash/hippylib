...
class TVPrior:
    ...
    def __init__(
        self,
        Vhm: dl.FunctionSpace,
        Vhw: dl.FunctionSpace,
        Vhwnorm: dl.FunctionSpace,
        alpha: float,
        beta: float,
        peps: float = 1e-3,
        rel_tol: float = 1e-12,
        ...
        self.varfM = self.peps * dl.inner(self.m_trial, self.m_test) * dl.dx  # mass matrix variational form for preconditioner, can be set by user
    ...
    def grad(self, m, out):
        ...
        TVm = self._fTV(m)
        grad_tv = self.alpha * dl.Constant(1.0) / TVm * dl.inner(dl.grad(m), dl.grad(self.m_test)) * dl.dx
    ...
    def primal_hess_coeff(self, m, w, TVm):
        ...
        if self.ncomp == 0:
            # only one parameter
            Acoeff = dl.Constant(1.0) / TVm * (dl.Identity(self.ndim) - dl.Constant(0.5) * dl.outer(w, dl.grad(m) / TVm) - dl.Constant(0.5) * dl.outer(dl.grad(m) / TVm, w))
        else:
            ...
            Acoeff = dl.Constant(1.0) / TVm * (eye - dl.Constant(0.5) * dl.outer(w, dl.grad(m) / TVm) - dl.Constant(0.5) * dl.outer(dl.grad(m) / TVm, w))
...
class weightedVTVPrior:
    ...
    def primal_hess_coeff(self, m, w, wvtv_form):
        ...
        Acoeff = (1.0 / wvtv_form) * (weighted_eye - dl.Constant(0.5) * dl.outer(self.alpha * w, self.alpha * v) - dl.Constant(0.5) * dl.outer(self.alpha * v, self.alpha * w))
